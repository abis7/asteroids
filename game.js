...
class Ship {
  ...
  draw() {
    ...
    ctx.strokeStyle = this.speedBoost > 0 ? '#4dd8ff' : '#fff';
    ctx.lineWidth   = 1.5;
    ctx.lineJoin    = 'round';
...
function drawHUD() {
  ...
  if (ship.shield > 0)
    drawPowerBar('ESCUDO', ship.shield / 5, '#00e5ff', 40);
  if (ship.speedBoost > 0)
    drawPowerBar('VELOCIDAD', ship.speedBoost / 5, '#ff9f0a', 40 + (ship.shield > 0 ? 30 : 0));
