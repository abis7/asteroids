...
- **State machine**: global `state` is `'playing' | 'dead' | 'gameover' | 'menu'` (`game.js:241`); update branches per state. `dt` is clamped to 0.05s in the loop.
...
- **Power-ups**: only one power-up on screen at a time; spawned at 15% per asteroid destroyed. Three types ~1/3 each: `'speed'` (orange `>>`, ship 2x thrust), `'shield'` (cyan shield icon, ship crushes asteroids on contact for 5 s) and `'triple'` (magenta fan icon, ship fires 3 spread bullets ±0.15 rad). All last 5 s via `ship.speedBoost`/`ship.shield`/`ship.tripleShot`; HUD bars via `drawPowerBar()` (`game.js`).
- **Skins**: ship look comes from the `SKINS` registry (each entry: `id`, `nombre`, `trazo`, `trazoBoost`, `trazoTriple`, `llama`, `pts` silhouette array, `escape` flame x). Global `selectedSkin`/`skinIndex`; persisted in `localStorage` key `asteroids_skin` (`loadSkin`/`saveSkin`). Only opened via `KeyS` from `'playing'`; while `state === 'menu'` the world update returns early (frozen) and `drawMenu()` draws a `rgba(0,0,0,0.85)` overlay. Always style ships/life icons via the skin's fields, never hard-code colors in `Ship.draw()`/`drawLifeIcon()`.
